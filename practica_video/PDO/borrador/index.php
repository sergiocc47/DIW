	...
	<body style="padding:20px;">	<!-- TODO: quitar padding a resto de elementos -->
		...
		<div id="formulario" style="border: 1px solid red;padding:20px;">
			<form method="get" action="index.php" >
				Introduzca un nuevo título <input type="text" name="titulo"/>
				<input type="submit" name="boton_modificar" value="Modificar título"/>
			</form>
		</div>
			<?php
				...
				if(isset($_GET['titulo']) && !empty($_GET['titulo'])){
					...
					if($c==null){
						...
						// ejecuto la consulta para mostrar el título
						//$sql="UPDATE titulos SET titulo='$_GET['titulo']' WHERE id_titulo='1'";	//FAIL
						$sql="UPDATE titulos SET titulo='".$_GET['titulo']."' WHERE id_titulo='1'";
						
						//***********************************************************
						//*********************** IMPORTANTE ************************
						//***********************************************************
						$registro = $c->exec($sql);		// query es sólo para SELECT; las demás exec
						
						if($registro==0){
							echo "<p>Sin cambios</p>";
						}
						else{
							// while ($registro = $resultado->fetch()) {
								// echo "<p class='titulo'>".$registro['titulo']."</p>";
							// }
							echo "<p>Cambio/s realizado/s</p>";
						}
